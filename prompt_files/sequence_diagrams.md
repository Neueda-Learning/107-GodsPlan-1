...
Notes
- The rate is fetched once, during the CREATED→VALIDATED step, and
  never re-fetched later in the same payment's lifecycle — it is
  frozen onto the payment record (`exchange_rate`, `exchange_rate_id`).
- ExchangeRateSvc caches `info.quote` (the unit rate), not `result` —
  `result` is only valid for this call's specific `amount=250`. A
  subsequent USD→INR payment for a *different* amount, within the
  cache TTL, skips the API call entirely and computes its own
  `destinationAmount` as `amount * 95.330968`.
- `info.timestamp` (Unix seconds) is converted to a `TIMESTAMP` before
  being stored as `exchange_rates.fetched_at`.
...
Notes
- Exactly one retry, then fail — never hang the request waiting on an
  external provider (architecture.md, ExchangeRateService rules).
- No `exchange_rates` row and no `destination_amount` are ever
  persisted for a failed lookup — the FAILED payment simply carries
  the error code, same pattern as every other validation failure.
