...
const primaryKpis = ['totalTransactions', 'totalPaymentVolume', 'successfulPayments', 'failedPayments', 'paymentSuccessRate', 'activeCustomers']
...
export default function Analytics() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-6">
    ...
    {error && <div className="rounded-card border border-red-200 bg-red-50 p-4 text-sm text-red-700" role="alert"><p>{error}</p><button className="mt-2 font-semibold underline" onClick={() => loadDashboard(filters)}>Retry</button></div>}
    ...
    {loading ? <div className="space-y-6"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }, (_, index) => <div className="card" key={index}><LoadingSkeleton rows={2}/></div>)}</div><div className="grid gap-6 xl:grid-cols-2"><div className="card"><LoadingSkeleton rows={5}/></div><div className="card"><LoadingSkeleton rows={5}/></div></div></div> : overview && <>
