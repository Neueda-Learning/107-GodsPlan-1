...
public class AnalyticsSeedService {
    ...
    private SeedCounts seedPayments(List<SeedAccount> accounts) {
        ...
        for (int index = 0; index < 120; index++) {
            ...
            BigDecimal seedFee = amount.multiply(new BigDecimal("0.02")).setScale(2, RoundingMode.HALF_EVEN);
            BigDecimal seedTotal = amount.add(seedFee);
            jdbc.update("INSERT INTO payments (idempotency_key, amount, fee_amount, total_debit_amount, currency, source_account_id, destination_account_id, "
                            ...
                            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)", key, amount, seedFee, seedTotal, account.currency,
